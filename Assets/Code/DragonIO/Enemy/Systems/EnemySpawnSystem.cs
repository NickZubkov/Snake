        ...
        public void Run()
        {
            ...
            if (_enemy.IsEmpty())
            {
                foreach (var idx in _controller)
                {
                    ...
                    {
                        for (int i = 0; i < dragonsConfig.EnemyCount; i++)
                        {
            ...
            foreach (var signal in _spawnedSignal)
            {
                    ...
                    var bodyWithLegs = Object.Instantiate(enemy.EnemyConfig.BodyPrefabFrontLegs, position, Quaternion.identity);
                    ...
                    bodyWithLegs = Object.Instantiate(enemy.EnemyConfig.BodyPrefabBackLegs, position, Quaternion.identity);
                    bodyWithLegs.Spawn(_world.NewEntity(), _world);
                    bodyWithLegs.SetComponentReferences(_spawnedSignal.GetEntity(signal));
                    dragonHead.BodyParts.Add(bodyWithLegs.transform); 
