...
float4 frag(v2f i) : SV_Target
{
	...
	fixed lightDot = clamp(dot(i.worldNormal, lightDir), -1, 1);
				
	lightDot = exp(-pow(_K*(1 - lightDot), _P));
	fixed4 light = lerp(_DarkColor, _BrightColor, lightDot);

	fixed shadowColor = LIGHT_ATTENUATION(i);
	fixed4 finalColor = _Color * light * shadowColor;
